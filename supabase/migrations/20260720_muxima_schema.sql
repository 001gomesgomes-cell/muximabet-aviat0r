...
-- ===== BÓNUS DE BOAS-VINDAS (1000 Kz, resgatado 1x pelo lead) =====
...
begin
  ...
  -- autoriza a alteração de saldo (guard_profile_money)
  perform set_config('app.balance_ok', 'on', true);

  select welcome_bonus_claimed, balance into v_claimed, v_new_balance
    from public.profiles where user_id = v_uid for update;
  if not found then
    return jsonb_build_object('status', 'error', 'message', 'no_profile');
  end if;

  if v_claimed then
    return jsonb_build_object('status', 'already_claimed', 'new_balance', v_new_balance);
  end if;
  ...
  return jsonb_build_object('status', 'credited', 'bonus', 1000, 'new_balance', v_new_balance);
end;
$$;
